...
<div>
          ...
          <tr>
            <th>
              ...
              <template v-for="(item, props, index) in headers">
                <th :key="index" scope="col">{{$t('positionAdmin.'+props)}}</th>
              </template>
          </tr>
          </thead>
          <tbody>
  ...
  <div>
    <el-dialog title="Vị trí công việc" class="transition-box-center" :visible.sync="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false">
      <div>
        <div class="form-group">
          <label class="control-label font-weight-bold">Tên vị trí</label>
          <div class="mb-20">
            <input type="text" class="form-control" placeholder="Nhập tên vị trí" v-model="formData.name" />
          </div>
        </div>
        <div class="form-group">
...
export default {
  ...
  computed: {
    ...mapState({
      headers: state => state.$_positionAdmin.headers,
      searchRequest: state => state.$_positionAdmin.searchRequest,
    }),
    ...mapGetters({
        data: "$_positionAdmin/getData",
        departmentList: "$_positionAdmin/getDepartmentList",
    }),
  },
  watch: {
  ...
  async created() {
    var _this = this;
    const STORE_KEY = "$_positionAdmin";
    if (!(STORE_KEY in this.$store._modules.root._children)) {
      _this.$store.registerModule(STORE_KEY, store);
    }
    await _this.$store.dispatch("$_positionAdmin/getDepartmentList");
    await _this.$store.dispatch("$_positionAdmin/getData");
  },
  methods: {
    async handleSearch(){
      var _this = this;
    ...
    submit: _.debounce(async function () {
      var _this = this;
      try {
        await _this.$store.dispatch("$_positionAdmin/editPosition", _this.formData);
        _this.$notify({
          title: "Chúc mừng",
