...
const getMessageOfConversation = async (pageIndex, pageSize, conversationId) => {
  ...
  return Vue.prototype.$http.post(`http://172.20.10.25:5000/api/conversation/getMessageOfConversation`, query);
}

const sendMessage = async (request) => {
  return Vue.prototype.$http.post(`http://172.20.10.25:5000/api/conversation/sendMessage`, request);
}

const createConversation = async (request) => {
  return Vue.prototype.$http.post(`http://172.20.10.25:5000/api/conversation/create`, request);
}
...
const getStickerList = async () => {
  return Vue.prototype.$http.get(`http://172.20.10.25:5000/api/stickerGroup/getAll`);
}
